...
begin inputs
	...
	color incandescence
	float incandescence_intensity

	color reflect_color
	float reflect_roughness
	float reflect_samples
	float reflect_reflectivity
...
vector DL_ADV_TransformToTangentSpace(vector i_v; vector i_stangent; normal i_normal)
{
	...
	/*
		NOTE: I think we can ignore the i_v[2] * i_normal term here.
		This is because the anisotrpy direction will be projected onto
		the surface anyway. Some basic test did confirm this but I am
		still leaving it here in case I am missing something.
	*/
...
{
	/* In RT SSS the rays are traced from inside the model. In this case the
	 * model should looks like it rendered with normal way from the outside. */
	float vdotn = clamp(abs( V.Nn), EPSILON, 1.0);
	float ldotn = clamp(abs(Ln.Nn), EPSILON, 1.0);
		...
		output color o_reflection_environment;)
{
	...
	{
		/* Refraction */
		trace_dir = refract( In, i_normal, ior);
	...
	else
	{
		...
		if( is_refraction == 1 )
		{
			 ...
			 * in standard trace() */
			W_o = reflect(-trace_dir, -i_normal);
			__N = -i_normal;
		}
		else
		{
		...
		output color o_outTransparency;)
{
	...
	if( i_coating_on == 1 )
	{
		...
		coating_ior = eta_from_intensity( i_coating_reflectivity );
#if 0
		...
		fresnel(In, Nn_coating, 1/coating_ior, coating_fresnel, __tmp_buf);
		...
		/* Get area light specular */
		if( i_coating_has_specular != 0 &&
			coating_color != 0 &&
			__is_shadow_ray != 1 &&
			__is_subsurface_ray != 1 &&
			__is_irradiance_ray != 1 )
		{
			...
			trace(
				...
				"bsdf", "blinn",
				"wo", V,
				"roughness", i_coating_roughness,
				"weight", coating_color );
			coating_specular += clight;
		}
	...
	/* Get area light specular */
	if( i_reflect_has_specular != 0 &&
		reflect_color != 0 &&
		__is_shadow_ray != 1 &&
		__is_subsurface_ray != 1 &&
		__is_irradiance_ray != 1 )
	{
		if( i_reflect_anisotropy == 0 ||
			anisotropy_stangent == 0 ||
			anisotropy_direction == 0 )
		{
			color clight;
			trace(
					P, Nn_base,
					...
					"bsdf", "blinn",
					"wo", V,
					"roughness", i_reflect_roughness,
					"weight", reflect_color );
			reflect_specular += clight;
		}
		else
		{
			float anisotropy = 1/(1-0.75*abs(i_reflect_anisotropy));
			if( i_reflect_anisotropy < 0 )
			{
				anisotropy = 1/anisotropy;
			}
			...
			trace(
					...
					"wo", V,
					"roughness", i_reflect_roughness * anisotropy,
					"roughnessv", i_reflect_roughness / anisotropy,
					"udir", anisotropy_udir,
					"weight", reflect_color );
			reflect_specular += clight;
		}
		...
		__is_subsurface_ray != 1 )
	{
		refract = DL_ADV_GetReflectionRefractionLayer(
				...
				i_refract_fog_strength,
				0, 0, 0,
				/* Provide faceforwarded normal to the refration.
				 * Refraction needs faceforwarded normal because it uses
				 * distribution for reflection and it's hacked to be refraction.
				 * IOR is already depends on front/backside so rays will
				 * leave the object in a correct direction. */
				Nf_base,
				refract_samples,
				i_refract_model,
	...
	if( diffuse_color != 0 )
	{
		if( i_sss_on != 1 || __is_subsurface_ray == 1 )
		{
			...
			indirect_diffuse = getGlobalIlluminationComponents(
				Nn_base, indirect_diffuse_weight,
				gi_visibility, gi_env_diffuse, gi_color_bleeding );
// #else
			// indirect_diffuse =
			// 	getIndirectDiffuse( Nn_base, indirect_diffuse_weight );
#endif
		}
		else
		{
	...
	{
		aov_diffuse +=
			__transparency * diffuse * diffuse_color;
	}
#endif
	...
	{
		aov_gi +=
			__transparency * indirect_diffuse * diffuse_color * coating_absorbed;
	}
#endif

#if defined(USE_AOV_aov_occlusion) || \
	defined(USE_AOV_aov_env_diffuse) || \
	defined(USE_AOV_aov_indirect)
#ifdef USE_AOV_aov_occlusion
	extern color aov_occlusion;
	if( isoutput( aov_occlusion ) )
	{
		aov_occlusion +=
			__transparency * gi_visibility * opacity;
	}
#endif
#ifdef USE_AOV_aov_env_diffuse
	extern color aov_env_diffuse;
	if( isoutput( aov_env_diffuse ) )
	{
		aov_env_diffuse +=
			__transparency * gi_env_diffuse * diffuse_color * coating_absorbed;
	}
#endif
#ifdef USE_AOV_aov_indirect
	extern color aov_indirect;
	if( isoutput( aov_indirect ) )
	{
		aov_indirect +=
			__transparency * gi_color_bleeding * diffuse_color * coating_absorbed;
	}
#endif
#endif
	...
	if( isoutput( aov_incandescence ) )
	{
		aov_incandescence +=
			__transparency * incandescence * coating_absorbed;
	}
#endif

#ifdef USE_AOV_aov_subsurface
	extern color aov_subsurface;
	if( isoutput( aov_subsurface ) )
	{
		aov_subsurface +=
			__transparency * sub_surface * diffuse_color * coating_absorbed;
	}
#endif
	...
	{
		aov_specular +=
			__transparency * coating_specular * coating_color +
			__transparency * reflect_specular * reflect_color;
	}
#endif
	...
	{
		aov_reflection +=
			__transparency * coating * coating_color +
			__transparency * reflect * reflect_color * coating_absorbed;
	}
#endif
#ifdef USE_AOV_aov_rt_reflection
	...
	{
		aov_rt_reflection +=
			__transparency * coating_reflect_object * coating_color +
			__transparency * reflect_object * reflect_color * coating_absorbed;
	}
#endif
#ifdef USE_AOV_aov_env_reflection
	...
	{
		aov_env_reflection +=
			__transparency * coating_reflect_environment * coating_color +
			__transparency * reflect_environment * reflect_color * coating_absorbed;
	}
#endif
	...
	{
		aov_refraction +=
			__transparency * refract * refract_color * coating_absorbed;
	}
#endif
	...
	{
		aov_luminance_depth = computeLuminanceDepth() * opacity;
	}
#endif
	...
	{
		aov_surface_color += __transparency * diffuse_color;
	}
#endif

	normal Nf_output = i_coating_on == 1 ? Nf_coating : Nf_base;
#ifdef USE_AOV_aov_camera_space_normal
	extern normal aov_camera_space_normal;
	if( isoutput( aov_camera_space_normal ) )
	{
		aov_camera_space_normal = Nf_output;
	}
#endif
#ifdef USE_AOV_aov_world_space_normal
	extern normal aov_world_space_normal;
	if( isoutput( aov_world_space_normal ) )
	{
		aov_world_space_normal = ntransform("world", Nf_output);
	}
#endif
#ifdef USE_AOV_aov_motion_vector
	extern vector aov_motion_vector;
...
#endif
#ifdef USE_AOV_aov_facing_ratio
	extern float aov_facing_ratio;
	if( isoutput( aov_facing_ratio ) )
	{
		aov_facing_ratio = Nf_output.V;
	}
#endif
	...
	// RGBA_SET_OUTPUT(
	// 		Specular,
	// 		coating_specular * coating_color + reflect_specular * reflect_color,
	// 		alpha );

	// RGBA_SET_OUTPUT(
	// 		Reflection,
	// 		coating * coating_color + reflect * reflect_color * coating_absorbed,
	// 		alpha );
